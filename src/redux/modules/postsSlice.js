...
export const __getPostsSlice = createAsyncThunk('posts/getPostsSlice', async (payload, thunkAPI) => {
  try {
    ...
    // // const writerNicknames = posts.map((post) => {
    // //   const writerProfile = profiles.find((profile) => profile.uid === post.uid);
    // //   return writerProfile ? writerProfile.nickname : '';
    // // });
    // const writerNicknames = posts.map((post) => {
    //   const writerProfiles = profiles.filter((profile) => profile.uid === post.uid);
    //   return writerProfiles.length > 0 ? writerProfiles[0].nickname : '';
    // });
...
export const postsSlice = createSlice({
  ...
  extraReducers: {
    [__getPostsSlice.fulfilled]: (state, action) => {
      ...
      // state.writerNicknamesData = action.payload.writerNicknames;
    },
